...
void StateFull::Update(double dt)
{
	...
	if (m_go->nearest == nullptr || !m_go->nearest->active)
	{
		time += dt;
		if (time >= 2.f) {
			PostOffice::GetInstance()->Send("Scene", new MessageWRU(m_go, MessageWRU::SEARCH_TYPE::NEAREST_SHARK, 0));
			time = 0;
		}
		//std::cout << time;
...
void StateHungry::Update(double dt)
{
	...
	{
		if (time >= 2.f) {
			PostOffice::GetInstance()->Send("Scene", new MessageWRU(m_go, MessageWRU::SEARCH_TYPE::NEAREST_FISHFOOD, 0));
			time = 0;
		}
	}
	time += dt;
